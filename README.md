...
and Vincent Rijmen. AES was designed to be efficient in both hardware and
...
In order to implement a DPA attack, an attacker first observes m encryption
operations and T power traces captures with k samples each.
...
The first exercise was to plot the power traces and then 'zoom' on the first
round :
```Matlab
...
those attacks. The consumption randomisation and reduction is clearly too
